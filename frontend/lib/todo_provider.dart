...
class TodoProvider with ChangeNotifier {
  ...
  Future<void> fetchTodayTodosGrouped() async {
    ...
    final response = await http.get(
      Uri.parse("${Env.baseUrl}/plan/by-date-with-subject?date=$todayStr"),
  ...
  Future<void> toggleTodayCheck(int planId, bool value) async {
    ...
    final url = Uri.parse("${Env.baseUrl}/plan/$planId/complete");
  ...
  Future<void> fetchTodosFromDB({int retry = 0}) async {
    ...
    final response = await http.get(
      Uri.parse("${Env.baseUrl}/plan/weekly-grouped"),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
